...
import siteConfiguration from './.f1/make/site.json'

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
  // .f1/make/deploy-preview passes `--mode development` for cached-preview builds.
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "CyberShield"
